...
st.write("""
### Klasifikasi tingkat tingkat kelebihan cairan pasien transfusi darah menggunakan Metode Decision tree, Random forest, dan SVM
""")
st.write('## 1. Introduction')
st.write('Transfudi darah adalah proses pemindahan darah dari seseorang yang sehat (donor) ke orang sakit (resipien), dimana darah yang dipindahkan dapat berupa darah lengkap dan komponen darah.')
...
st.write('## 2. About Dataset (Blood Transfusion)')
data_hf = pd.read_csv("https://raw.githubusercontent.com/ranianuraini/datminweb/main/transfusion.csv")
st.write("Dataset Heart Failure : (https://raw.githubusercontent.com/ranianuraini/datminweb/main/transfusion.csv) ", data_hf)

st.write('Dataset Description :')
st.write('1. Recency (months)')
st.write('2. Frequency (times)')
st.write('3. Monetary (c.c. blood)')
st.write('4. Time (months)')
st.write('5. whether he/she donated blood in March 2007(Boolean)')
...
Recency = st.sidebar.number_input("Recency (months) =", min_value=0, max_value=75)
Frequency = st.sidebar.number_input("Frequency (times)  =", min_value=0, max_value=50)
Monetary = st.sidebar.number_input("Monetary (c.c. blood) =", min_value=0, max_value=11000)
Time  = st.sidebar.number_input("Time (months) =", min_value=0, max_value=98)
whether = st.sidebar.number_input("whether he/she donated blood in March 2007 =", min_value=0, max_value=0)
submit = st.sidebar.button("Submit")

if submit :
    if algoritma == 'Decision Tree' :
        X_new = np.array([[Recency, Frequency, Monetary, Time, whether]])
        prediksi = decision_tree.predict(X_new)
        if prediksi == 1 :
            st.sidebar.write(""" # Hasil Prediksi :
             resiko kelebihan cairan tinggi""")
        else : 
            st.sidebar.write("""# Hasil Prediksi :
             resiko kelebihan cairan rendah""")
